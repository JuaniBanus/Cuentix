...
-- 3. Recien ahi seleccionar la PARTE 2 y apretar Run.
--
...
declare
    faltan text;
begin
    select string_agg(distinct m.tipo || '/' || m.hacia, ', ')
      into faltan
      from mapa_categorias m
     where not exists (
         select 1 from public.categorias c
          where c.user_id is null and c.tipo = m.tipo and c.nombre = m.hacia
     );
