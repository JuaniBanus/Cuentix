...
//
// POR QUÉ UNA FÁBRICA Y NO UN OBJETO SUELTO
// Casi todo lo que hay acá adentro son datos de UNA persona, y al cerrar sesión
// no puede quedar nada. Antes el reinicio era una lista de asignaciones escrita
// a mano, y esa lista se desincronizó: `rendimientos` y `falloCotizacion` se
// agregaron arriba y nunca se agregaron abajo. Esta vez zafamos porque las dos
// son públicas, pero el próximo campo que se olvide puede no serlo.
//
// Con la fábrica no hay dos listas que mantener: agregar un campo acá ya lo
// deja incluido en el reinicio, sin que haya que acordarse de nada.
...
function estadoInicial() {
  return {
    ...
    // TNA de las billeteras virtuales, para el comparador de Ahorros. Las trae un
    // cron del backend; la web solo las lee. [] = todavía no llegaron o falló.
    rendimientos: [],
...
// Una sola instancia, que todos los módulos importan por referencia. Por eso el
// reinicio la RELLENA en lugar de reemplazarla: si acá se reasignara el objeto,
// los módulos que ya lo importaron seguirían mirando el viejo.
...
export function reiniciarEstado() {
  ...
  // Se borran también las claves que no estén en la fábrica. Hoy no hay
  // ninguna, pero un `estado.loQueSea = ...` escrito desde otro módulo no
  // tendría por qué sobrevivir a un cambio de usuario.
