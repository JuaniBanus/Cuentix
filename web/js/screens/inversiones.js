...
import { esc, fechaCorta, monto, montosOcultos } from "../format.js";
...
/** Número con 2 decimales y separadores locales, respetando el ojo. */
function pct(valor) {
 ...
 */
function agrupar(posiciones, clave, cotizaciones) {
  const mapa = new Map();

  for (const p of posiciones) {
    const tasa = p.moneda === "USD" ? 1 : cotizaciones?.tasas?.[p.moneda];
...
function filaPosicion(p) {
  ...
  const subtitulo = p.ticker ? esc(p.nombre) : ETIQUETA_TIPO[p.tipo];
  ...
  const bloqueGP = p.valuada
    ...
    : `<span class="posicion-gp es-sin-dato" title="El precio de mercado de acciones, CEDEARs, bonos y FCI llega en el Sprint 7. Por ahora se muestra el precio de compra.">
         sin precio aún
       </span>`;

  return `
    <li class="posicion">
      <span class="posicion-marca" aria-hidden="true">${esc(identidad.slice(0, 4))}</span>
...
export function renderInversiones(
  contenedor,
  inversiones,
  { precios, errorPrecios, sinCotizar, cotizaciones }
) {
  ...
  if (sinValuar) {
    avisos.push(
      ...
      `(acciones, CEDEARs, bonos y FCI: precio de mercado en el Sprint 7)`
    );
  }
  ...
  const porTipo = agrupar(posiciones, (p) => ETIQUETA_TIPO[p.tipo] ?? p.tipo, cotizaciones);
  ...
  const porMoneda = agrupar(posiciones, (p) => p.moneda, cotizaciones);
  contenedor.querySelector("#barras-moneda").innerHTML = porMoneda
    ? `<ul class="barras">
        ${porMoneda.map((m, i) => `
          <li class="barra-fila">
            <span class="barra-nombre">${esc(m.categoria)}</span>
            <span class="barra-pista">
              <span class="barra-relleno" style="width:${m.porcentaje.toFixed(1)}%;
                    background:${colorDeCategoria(i)}"></span>
            </span>
            <span class="barra-monto">${monto(m.total, "USD")}</span>
            <span class="barra-pct">${m.porcentaje.toFixed(0)}%</span>
          </li>`).join("")}
      </ul>`
    : SIN_TASAS;
}
